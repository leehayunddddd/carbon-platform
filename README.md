...
대시보드 UI 구현에도 시간이 많이 소요됐습니다. 단순히 데이터를 표시하는 것에서 나아가 실무자와 경영자가 한눈에 파악할 수 있도록 정보 구조와 레이아웃을 고민하는 과정이 필요했습니다.
...
- 활용 방식 및 판단  
  ...
  특히 배출계수 변경 시 수정 범위를 최소화하기 위해 lib/pcf.ts에서 계산 로직을 관리하도록 구성했습니다.
...
- 활용 방식 및 판단  
  ...
  이후 데이터 가독성과 사용자 흐름을 고려하여 카드 구성과 섹션 배치를 수정한 뒤 실제 UI로 구현했습니다다

  ### API 설계

  - 사용 목적
    계산 로직을 서버사이드로 분리하기 위해 AI를 활용했습니다.

  - 사용 Prompt
    Next.js API Route로 PCF 계산 로직을 서버사이드 API로 분리

  - 활용 방식 및 판단
    AI가 제안한 API route 구조를 참고하여 POST /api/calculate 엔드포인트를 구현했습니다.
    클라이언트에서 엑셀 파싱 후 데이터를 API로 전송하고, 서버에서 PCF 계산 후 결과를 반환하는 구조로 설계했습니다.
    현재는 DB 없이 계산 결과만 반환하지만, DB 연동 시 route.ts 한 파일만 수정하면 되는 확장 가능한 구조로 판단했습니다.

  ### 6. 컴포넌트 분리 및 리팩토링

- 사용 목적
  코드 재사용성과 유지보수성을 높이기 위해 AI를 활용했습니다.

- 사용 Prompt
  차트 컴포넌트를 역할별로 분리하고 배출계수를 constants로 관리

- 활용 방식 및 판단
  AI 제안을 참고하여 MonthlyBarChart, TypePieChart, TrendLineChart로 분리했습니다.
  배출계수 중복 정의 문제를 constants.ts로 단일화하여 한 곳만 수정하면 되도록 개선했습니다.
